...
import { handleClearSlashCommand } from "@/lib/slack/slash-commands";
...
export async function POST(request: Request) {
  try {
    ...
    const ack = await handleClearSlashCommand(payload);
    return NextResponse.json(ack);
  } catch (error) {
    if (error instanceof AppError && error.statusCode === 401) {
      return jsonError(error, "POST /api/slack/commands/clear");
    }
    return NextResponse.json(
      {
        ...
        text:
          error instanceof AppError && error.expose
            ? error.message
            : "Unable to clear the Baxter conversation.",
      },
      { status: 200 },
    );
  }
