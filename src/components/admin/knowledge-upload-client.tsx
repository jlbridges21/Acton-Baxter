...
export function KnowledgeUploadClient() {
  ...
  async function runPreview() {
    ...
    try {
      ...
      setMessage(`Preview ready for ${rows.length} file(s).`);
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-[var(--acton-navy)]">Upload documents</h1>
        <p className="mt-1 text-sm text-[var(--acton-muted)]">
          Supported: .md, .txt, .pdf, .docx, .csv, .xlsx. OCR is not supported for scanned PDFs.
        </p>
      </div>
          ...
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <CardDescription className="mt-1">
                {preview.extension.toUpperCase()} · {preview.extractionStatus} · {preview.wordCount}{" "}
                words · {preview.characterCount} chars
              </CardDescription>
            </div>
