 ...
 * Soft UX helper — never throws on Slack API failure (same convention as
 * addSlackReaction). Callers must check `result.ok` and log `result.error`.
...
export async function postEphemeralSlackMessage(
  ...
  try {
    ...
    if (!input.channel || !input.user) {
      return { ok: false, error: "missing_channel_or_user" };
    }
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      const response = await fetch("https://slack.com/api/chat.postEphemeral", {
        ...
        body: JSON.stringify({
          ...
          text: input.text,
          ...(input.blocks ? { blocks: input.blocks } : {}),
          // Only pass thread_ts when the caller supplies a *parent* ts.
          // Never pass a reply message's own ts (Slack: invalid_thread_ts /
          // ephemeral may not display). See chat.postEphemeral docs.
          ...(input.threadTs ? { thread_ts: input.threadTs } : {}),
        }),
      });
    ...
    return { ok: false, error: "ephemeral_exception" };
  }
}
