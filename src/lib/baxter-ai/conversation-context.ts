...
const NEW_SUBJECT =
  /\bhow much (have|did|do) we (sold|sell)\b|\bwhat (is|are) (our|the)\b|\bwho (manages|handles|owns)\b|\bfeasibility process\b|\bpermitting process\b|\bwarranty\b|\bbuild ready process\b|\bhow many (projects|contracts)\b|\bwhat happens\b|\bexplain\b/i;

/** Aggregation / company totals. */
const AGGREGATION_INTENT =
  /\bhow much (have|did|do) we\b|\bhow many (projects|contracts|deals)\b|\btotal (sales|agreement|sold)\b|\baverage agreement\b|\bwe sold\b|\bsold this (year|month)\b|\bsold (in|during)\b/i;
